...
class userInfoService {
     ...
     */
     async userInfoChecked(id) {
        const statement = `SELECT * FROM tb_userInfo WHERE id = ?;`
    ...
    async userInfoUpload(id, schoolName, collage, studentId, phone) {
        ...
        if(result != null && result != 0 && result != []){
            //若查询成功有该用户信息
            const statement = `UPDATE tb_userInfo SET schoolName = ?, collage = ?, studentId = ?, phone = ? WHERE id = ?;`
            const [result] = await connection.execute(statement,[schoolName, collage, studentId, phone, id]);

        }else{
            //若查询不到该用户信息
            const statement = `INSERT INTO tb_userInfo (id, schoolName, collage, studentId, phone) VALUES(?,?,?,?,?);`
